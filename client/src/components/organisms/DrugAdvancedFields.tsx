...
export const DrugAdvancedFields: React.FC<DrugAdvancedFieldsProps> = ({
    ...
    const filteredDosages = dosageSearch
        ? DOSAGE_FORMS.filter((d) => d.toLowerCase().includes(dosageSearch.toLowerCase()))
        : DOSAGE_FORMS;
    ...
    return (
            ...
            <div className="bg-gray-50 rounded-lg p-4 border border-gray-100 mb-2">
                ...
                <div className="mt-1 relative rounded-md shadow-sm">
                    ...
                    <input
                        ...
                        onBlur={() => setTimeout(() => setShowDosageResults(false), 100)}
                        autoComplete="off"
                    />
                    {showDosageResults && (
                        <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded mt-1 max-h-40 overflow-y-auto shadow-lg">
                            {filteredDosages.length === 0 ? (
                                <li className="px-3 py-2 text-gray-500 cursor-pointer" onMouseDown={() => setDosageSearch(dosageSearch)}>
                                    Add "{dosageSearch}"
                                </li>
                            ) : (
                                filteredDosages.map((d) => (
                                    <li
                                        key={d}
                                        className={`px-3 py-2 cursor-pointer hover:bg-blue-100 ${d === dosageSearch ? 'bg-blue-50 font-semibold' : ''}`}
                                        onMouseDown={() => setDosageSearch(d)}
                                    >
                                        {d}
                                    </li>
                                ))
        ...
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-100 mb-2 flex items-center">
            ...
            >
                <FaCheckCircle className="mr-1 text-green-500" /> Able to Sell
            </label>
        </div>
        {/* Drugs in Carton */}
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-100 mb-2">
        ...
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-100 mb-2">
            <label
                ...
                className="text-sm font-medium text-gray-700 flex items-center mb-1"
            >
                <FaLayerGroup className="mr-1 text-blue-500" /> Packs per Carton
                *
            </label>
            <div className="mt-1 relative rounded-md shadow-sm">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
