...
export const ReportChart: React.FC<ReportChartProps> = ({
    ...
    const renderChart = () => {
        switch (reportType) {
            case 'sales':
                return <SalesChart data={prepareSalesData(data)} />;

            case 'inventory':
                return <InventoryChart data={prepareInventoryData(data)} />;

            case 'expiry':
                return <ExpiryChart data={prepareExpiryData(data)} />;

            case 'financial':
                return <FinancialChart data={prepareFinancialData(data)} />;
